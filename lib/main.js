...
const integrationFunc = async (provider, context, scopes) => {
    ...
    if (url.pathname === "/logout") {
        const logoutURL = new URL("/logout", url.origin)
        return {
            res: await env.acheron.fetch(logoutURL)
        }
    }
    ...
    if (url.pathname.startsWith("/login/") === true) {
        ...
        res.headers.append(
            "location",
            new URL(authConfig.loginDest, authConfig.appOrigin).href
        )
